...
import static twitter.utilities.Tweets_delincuencia.callURL;

public class Tweets_agujeros {

    public List<BPunto_Agujero> get_tweets(List<BPunto_Agujero> list_in_db) { //list_in_db= listado de tweets en la base de datos

        List<BPunto_Agujero> list = new LinkedList<BPunto_Agujero>();
        String jsonString = callURL("https://search.twitter.com/search.json?q=%23agujero&include_entities=1&&geocode=-13.16074,-74.22563,1mi");
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length() - 1; i++) {   // iterate through jsonArray 
                ...
                if (!bandera) {//haora ya verifica si perteence o no  pra el registro en la base de datos
                    //BPunto_Agujero bPunto = new BPunto_Agujero();
                    BPunto_Agujero bPunto = new BPunto_Agujero();
                  //  BUsuario bUsuario = new BUsuario();
                    ...
                    bPunto.setTipo("Agujero Negro");
                    bPunto.setCalle("Calle Principal");
