    ...
    function mapData(f) {
        ...
        interaction.formatter(function(feature) {
            var o = '<h5 class="popover-geo-title">' + feature.fecha + '</h5>' +
                    ...
                    '<img style="height: 120px; width:120px;   margin-right: 25px;" src="' + feature.url_img + '">' +
                    '</div>';
            var a_button = '<a  role="button" class="btn"  href="#detail" onclick="call_detail_aten(\'' + feature.idpunto + '\')"> Más Detalle</a>';
            return o + a_button;
        });
...
function call_detail_aten(id) {
    ...
    $('#id_punto').val(f.idpunto);
    $('#id_punto').val(f.idpunto);
    $('#id_punto').attr('disabled', 'disabled');
    $('#button_submit').attr('disabled', 'disabled');
    $('#mensaje_alert').html('<div class="alert alert-success"> <strong>Punto Eliminado!</strong> ...</div>');
